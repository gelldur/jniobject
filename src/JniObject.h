...
private:
	...
	template<typename Arg, typename... Args>
	static void processObjectArguments(std::vector<jobject*>& jobjs
			, jvalue* jargs
			, unsigned pos
			, const Arg& arg
			, const Args& ... args)
...
public:
	...
	template<typename Return, typename... Args>
	Return
	staticCallSigned(const std::string& name, const std::string& signature, const Return& defRet, Args&& ... args)
	{
		JNIEnv* env = getEnvironment();
		if (!env)
		{
	...
	*/
	...
	static bool convertFromJavaMap(JNIEnv* env, jobject obj, std::map<Key, Value>& out)
	{
		...
		std::vector<Key> keys = jkeys
				.callSigned<std::vector<Key>>("toArray", "()[Ljava/lang/Object;", std::vector<Key>());
	...
	static JniObject createJavaMap(const std::map<Key, Value>& obj, const std::string& classPath = "java/util/HashMap")
	{
		...
		{
			Value v = jmap
					.callSigned("put", "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;", Value(), itr->first
								, itr->second);
		}
		return jmap;
	}
